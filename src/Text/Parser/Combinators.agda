...
module _ {P : Parameters}
 ...
 module _ {A B : Set} where
  ...
  _<&M>_ : ∀[ Parser P A ⇒ const (P.M B) ⇒ Parser P (A × B) ]
  runParser (A <&M> B) m≤n s =
    runParser A m≤n s 𝕄.>>= λ rA → B 𝕄.>>= λ b →
    𝕄.return (S.map (_, b) rA)

  _<&M_ : ∀[ Parser P A ⇒ const (P.M B) ⇒ Parser P A ]
  A <&M B = proj₁ <$> (A <&M> B)

  _&M>_ : ∀[ Parser P A ⇒ const (P.M B) ⇒ Parser P B ]
  A &M> B = proj₂ <$> (A <&M> B)
  ...
  _<M&>_ : ∀[ const (P.M A) ⇒ Parser P B ⇒ Parser P (A × B) ]
  runParser (A <M&> B) m≤n s =
    A 𝕄.>>= λ a → S.map (a ,_) 𝕄.<$> runParser B m≤n s

  _<M&_ : ∀[ const (P.M A) ⇒ Parser P B ⇒ Parser P A ]
  A <M& B = proj₁ <$> (A <M&> B)

  _M&>_ : ∀[ const (P.M A) ⇒ Parser P B ⇒ Parser P B ]
  A M&> B = proj₂ <$> (A <M&> B)
